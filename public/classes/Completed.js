...
export class CompletedTasks extends UnCompletedTasks {
    ...
    rederCompledTodos() {
        const outputComplete = document.querySelector(".complete");
        if (this.todo.filter((item) => item.IsCompleted == true).length == 0) {
            ...
            outputComplete.innerHTML = this.todo
                ...
                if (t.IsCompleted) {
                    ...
                    const d2 = new Date(t.completionDate); // expected Completion date
              ...
              <span> <button type="button" class="mark"  data-id="${t.id}">${t.IsCompleted ? "Mark UnCompleted" : "Mark Completed"} </button></span></div>
